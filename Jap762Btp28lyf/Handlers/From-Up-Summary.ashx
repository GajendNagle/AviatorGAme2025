﻿<%@ WebHandler Language="C#" Class="From_Up_Summary" %>
...
public class From_Up_Summary : IHttpHandler {

    public HttpContext context; public HttpRequest request; public HttpResponse response;
    public bool sc;
    public static string msg, dtl, strJson, UserID = "", SrchTxt,FrmDt = "", ToDt = "", Inc = "", Psize = "20";
    ...
    public void ProcessRequest(HttpContext _context)
    {
        ...
        if (context.Request.Cookies["Tap190Nvw92mst"] != null)
        {
            ...
            //    SrchTxt = "";
            //}
            Inc = context.Request["IncTyp"].ToString();
            ...
            //if (context.Request["IncTyp"].ToString() != "")
            //{
            //    SrchTxt = context.Request["IncTyp"].ToString();
            //}
            if (context.Request["IncTyp"].ToString() != "")
            {
                SrchTxt = context.Request["IncTyp"].ToString();
            }
            else if (request.QueryString["IncType"] != null)
            {
                SrchTxt = request.QueryString["IncType"].ToString();
            }
    ...
    private int BindResult(int currentPage)
    {
        ...
        try
        {          //dbo.FromUp_Summary
                   //ds = objgdb.ByProcedure("dbo.FromUp_Summary_New", new string[] { "intPageSize", "intCurrentPage", "memid", "Filter" },
                   //    new string[] { Psize, Pno.ToString(), UserID.ToString().Trim(), SrchTxt.ToString() }, "intTotalRecords", out TotalRows, "ByDataset");
            ds = objgdb.ByProcedure("dbo.FromUp_Summary_New", new string[] { "intPageSize", "intCurrentPage", "memid", "Status","FormDate", "ToDate" },
           new string[] { Psize, Pno.ToString(), UserID.ToString().Trim(), SrchTxt.ToString(),FrmDt, ToDt  }, "intTotalRecords", out TotalRows, "ByDataset");
            //if (TotalRows > 0)
            //{
            dt = ds.Tables[0];
            int count = TotalRows;
