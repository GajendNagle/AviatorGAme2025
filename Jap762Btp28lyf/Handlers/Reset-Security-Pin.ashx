<%@ WebHandler Language="C#" Class="Reset_Security_Pin" %>
...
public class Reset_Security_Pin : IHttpHandler {
   ...
   public HttpContext context;
        ...
        try
        {
            if (Get_Token != "")
            {
                ds = objgdb.ByProcedure("dbo.Pro_PasswordRecovery", new string[] { "MemID","EmlMbl", "RecCode", "NewPWD", "RType" },
                    new string[] { "","", Get_Token.TrimEnd(), "", "GETSPEMAIL" }, "ByDataset");
    ...
    public void GiveHelpOrder()
    {
        try
        {
            ...
            {
                ds = objgdb.ByProcedure("dbo.Pro_PasswordRecovery", new string[] { "MemID", "EmlMbl", "RecCode", "NewPWD", "RType" },
                    new string[] { changing_memid.Trim (), changing_email.Trim(), changing_code.Trim(), new_password.Trim(), "UpdSecPin" }, "ByDataset");
                if (ds.Tables[0].Rows.Count > 0)
                {
