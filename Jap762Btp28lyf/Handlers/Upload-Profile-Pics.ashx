    ...
    private void UploadProfileImg()
    {
        if (context.Request.Files.Count > 0)
        {
            ...
            for (int i = 0; i < files.Count; i++)
            {
                ...
                if (file.ContentLength > maxFileLength)
                {
                    msg = "Maximum length Of uploading profile pic is 300 KB...only!";
                    sc = false;
                }
                else
                {
                      ...
                      chkfileEx.ToString().Trim() == ".png" || chkfileEx.ToString().Trim() == ".pdf")
                    {
                        ...
                        //{
                        Int32 Wdth = 500, hght = 500;
                        ////
                        Wdth = cropcords.Width;
                        hght = cropcords.Height;
                        if (Wdth > 500)
                        {
                            Wdth = 500;
                        }
                        ...
                        bitMap = new Bitmap(Wdth, hght, oimg.PixelFormat);
                        ...
                        oimg.Dispose();
                        ////
                        FileInfo file1 = new FileInfo(fpath);
                        if (file1.Exists)
                        {
                            //the problem ocurred because this, 
                            using (FileStream s = new FileStream(fpath, System.IO.FileMode.Open, System.IO.FileAccess.Read, System.IO.FileShare.None))
