...
//        $.getJSON('Handlers/Inner_Page_Summary.ashx', function (OrjsonDS) {
//            if (OrjsonDS.length === 0) {
//                $('#paytimer10').html('').css('color', '');
//            } else {
//                var countdownText = OrjsonDS[0].countd;

//                $('#paytimer10').html('RemainTime: ' + countdownText);

//                if (countdownText === 'Time Over' || countdownText == '' || countdownText == null) {
//                    $('#paytimer10').css('color', 'red');
//                    $('#Paymentdetailsshow').hide();
//                    $('#PaymentdetailsshowCAPI').hide();
//                    /*$('#showdetails').show();*/
//                } else {
//                    $('#paytimer10').css('color', 'green');
        ...
        function (OrjsonDS) {
            if (OrjsonDS.length == 0) {
                ...
                for (var i = 0; i < OrjsonDS.length; i++) {
                    ...
                    $('#fmin').html(OrjsonDS[i].Fmin + "<small style='font-size: 14px;'> $</small>");
                    $('#fmax').html(OrjsonDS[i].FMax + "<small style='font-size: 14px;'> $</small>");
                    $('#fmlt').html(OrjsonDS[i].FMlt + "<small style='font-size: 14px;'> $</small>");
                    ...
                    $('#CPStatus_txt').html(OrjsonDS[i].AddFundSts);
                    ...
                    $('#RefLink1').html('https://wintron.online/register.html?invitation-code=' + OrjsonDS[i].MemID);

                    $('#MemberId').html(OrjsonDS[i].MemID);
                    $('#totalcommision').html(OrjsonDS[i].TotalCommision + '<span style="font-size:20px"> $</span>');
                    ...
                    $("#sharewt").val(OrjsonDS[i].CompName + " - The most amazing part of becoming our agent is that if the quality of your invited downlines is high, you have the opportunity to offer them free bonuses. " + '' + "https://wintron.online/register.html?invitation-code=" + OrjsonDS[i].MemID);
                    ...
                    $('#TOTAL_DEPOSIT').html(OrjsonDS[i].TOTAL_DEPOSIT + '<span class="vert">$</span>');
                    $('#TotWithdrawal').html('' + OrjsonDS[i].WithdrawalTotal + '<span class="vert">$</span>');
                    ...
                    $('#FBald').html(OrjsonDS[i].FBal + "<span style='font-size:12px'> $</span>");
                    ...
                    $('#Minwith').html(OrjsonDS[i].Minwith + " $");
                    $('#MltWith').html(OrjsonDS[i].MltWith + " $");
                    $('#TodayWith').html(OrjsonDS[i].TodayWith);
                    $('#betwithdraw').html(OrjsonDS[i].betwithdraw + " $");
                    $('#txtTRONAddress').val(OrjsonDS[i].WithWalletAddressTRC20);
                    $('#txtBinanceAddress').val(OrjsonDS[i].AccountNoBEP20);

                    $('#MinAmt').html(OrjsonDS[i].MinAmt + "<span style='font-size:12px'> $</span>");

                    $('#botrInc').html(OrjsonDS[i].Dcrinc + "<span style='font-size:12px'> $</span>");
                    $('#totwbalnc').html(OrjsonDS[i].TmPsBal + "<span style='font-size:12px'> $</span>");
...
function TxnhashsubmitPWtxn0() {
    
    $.ajax({
        url: "Handlers/GetBalance_CrytoAPI.ashx",
        type: "POST",
        contentType: false,
        processData: false,
        dataType: "json",
...
function TxnhashsubmitPWtxn() {
   
    $.ajax({
        url: "Handlers/DepositeAutoConfirm_CAPI.ashx",
        type: "POST",
        contentType: false,
        processData: false,
        dataType: "json",
